 ...
 */
package com.amazon.aws.demo.identity.sns;

import java.util.List;

import com.amazon.aws.demo.identity.AlertActivity;
import com.amazon.aws.demo.identity.R;
import com.amazon.aws.demo.identity.sqs.SimpleQueue;
...
public class SnsSubscribe extends AlertActivity{
    ...
    private void wireSubmitButton(){
    	submitButton.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				String arn = (String)topicSpinner.getSelectedItem().toString();
				if(inputTextStatus == INPUT_TEXT_ON) {
					endpointInput.setVisibility(View.GONE);
					try {
						SimpleNotification.subscribe(arn, (String)protocolSpinner.getSelectedItem(), endpointInput.getText().toString());
						finish();
					} catch (Throwable e ) {
	    				setStackAndPost(e);					}
				} else {
					endpointSpinner.setVisibility(View.GONE);
					String url = (String)endpointSpinner.getSelectedItem().toString();
    ...
    private void clearUi(){
    	...
    	endpointIntro.setText("Waiting for 12 Seconds For Subscribe to Propagate. . .");
