...
public class SosActivity extends ABSNabludatelActivity {
	...
	public void onTellUsClick(View v) {
		...
		if (cloud.tryAuthenticate()) {
			String sos_report_text = ((EditText) findViewById(R.id.sos_report_text))
					.getText().toString();
			String sos_report_key = findViewById(R.id.sos_report_text).getTag()
					.toString();
			...
			if (!sos_report_text.equals("") && pollingPlace != -1L) {
				...
				serverResponce = cloud.postNewMessage(sos_report_key,
						sos_report_text, lat, lng, timestamp, -1 /* rowId */,
						pollingPlace);
				if (serverResponce != -1) {
	...
	protected Dialog onCreateDialog(int id) {
		...
		case DIALOG_SEND_SOS_BEGIN: {
			return ProgressDialog.show(this, "", "Отправляю...", true);
		}
	...
	public void onSosInfoClick(View v) {
		showInfoDialog(R.string.sos_hint);
	}
}
